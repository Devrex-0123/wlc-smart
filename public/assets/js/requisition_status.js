...
function filteredData() {
    ...
    const data = requests.filter((r) => {
        ...
        const hay = `${r.id} ${r.requester || ''} ${r.office || ''} ${r.items.join(' ')} ${r.suppliers.join(' ')}`.toLowerCase();
...
async function loadRequests() {
    try {
        const response = await fetch(`${API}?action=list_requests`, { credentials: 'include' });
...
statusCards.addEventListener('click', (e) => {
    ...
    window.location.href = `requisition_status_progress.php?rid=${encodeURIComponent(String(record.request_id))}&from=status`;
});

searchInput.addEventListener('input', () => { currentReqPage = 1; renderCards(); });
statusFilter.addEventListener('change', () => { currentReqPage = 1; renderCards(); });
sortDropdown.addEventListener('change', () => { currentReqPage = 1; renderCards(); });
