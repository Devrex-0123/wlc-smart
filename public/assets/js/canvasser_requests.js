/**
 * Canvasser — requisition list; View opens canvass sheet (abstract of quotation); requisition matrix link is on that page.
 */
(function () {
    const API = '../../app/api/canvasser_requests.php';
    const REQ_VIEWS_PREFIX = 'imrms_request_views_';
    const requestTableBody = document.getElementById('cvRequestTableBody');
    const searchInput = document.getElementById('cvReqSearch');
    const statusFilter = document.getElementById('cvReqStatus');
    const sortDropdown = document.getElementById('cvReqSort');
    const totalCount = document.getElementById('cvTotalCount');
    const pendingCount = document.getElementById('cvPendingCount');
    const ongoingCount = document.getElementById('cvOngoingCount');
    const completedCount = document.getElementById('cvCompletedCount');
    const prevReqBtn = document.getElementById('cvPrevReqBtn');
    const nextReqBtn = document.getElementById('cvNextReqBtn');
    const reqPageInfo = document.getElementById('cvReqPageInfo');
    ...
    function renderTable() {
        ...
        if (!data.length) {
            requestTableBody.innerHTML = `
                <tr>
                    <td colspan="8" style="text-align:center;padding:2rem;color:#64748b;">No requisitions are assigned to you for canvassing yet.</td>
                </tr>`;
        ...
        requestTableBody.innerHTML = pageRows
            .map((r, i) => requestRowHtml(r, start + i + 1))
            .join('');
    ...
    function requestRowHtml(r, rowNum) {
        ...
        <tr${rowClass}>
            ...
            <td>
                <div class="actions-cell">
                    <button type="button" class="edit cv-view-form-btn" data-request-id="${esc(String(r.request_id))}" title="Open canvass sheet">
                        <i class="fas fa-table-list"></i> View${indicatorHtml}
                    </button>
                </div>
            </td>
        </tr>`;
    ...
    function bindViewNav(container) {
        if (!container) return;
        container.addEventListener('click', (e) => {
            const btn = e.target.closest('.cv-view-form-btn');
            if (!btn) return;
            ...
            window.location.href =
                'dean_canvass_form.php?from=canvasser&request_id=' +
                encodeURIComponent(String(rid));
        });
    }

    bindViewNav(requestTableBody);
